...
int main(int argc, char *argv[])
{
    ...
    if (argc == 1)
    {
        printf("Usage:   %s ifName DstMacAddr \n",argv[0]);
        printf("Example: %s wlan0 00:7F:5D:3E:4A \n",argv[0]);
		printf("You need do stop this executable manually.\n");
        exit(0);
    }
	...
	/* Send packet */
    while(1){
		/*Cnt = the number of packets you want to send every time interval*/
		for(Cnt = 1; Cnt > 0; Cnt-- )
  		{	
			printf("packet sent!\n");
		...
		/*time interval between every group of packets*/
		sleep(1);
	}
